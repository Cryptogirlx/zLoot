...
export const testPluck = (tokenId: number, keyPrefix: string, sourceArray: any) => {
    ...
    if (greatness >= 19) {
        ...
        console.log(arrays.Traits.namePrefix[name1],arrays.Traits.nameSuffix[name2])
        if (greatness === 19) {
            output = arrays.Traits.namePrefix[name1] + " " + arrays.Traits.nameSuffix[name2] + " " + output
        }
        else {
            let name1 = BigNumber.from(rand).mod(arrays.Traits.namePrefix.length)
            let name2 = BigNumber.from(rand).mod(arrays.Traits.nameSuffix.length)
